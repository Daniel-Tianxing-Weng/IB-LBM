...
double gravity; // force density due to gravity (in positive x-direction)
...
void read_parameters() {
  ...
  std::string myValues[12];
  std::string myObjects[12];
  ...
  gravity = stof(myValues[7]);

  IBM_stencil = stoi(myValues[8]); // interpolation stencil; available values: 2, 4
  wall_num_nodes = stoi(myValues[9]); // number of IBM nodes along each wall
  wall_width = stof(myValues[10]); // channel width
  wall_stiffness = stof(myValues[11]); // IBM spring constant
