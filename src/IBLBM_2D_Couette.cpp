...
int Nx; // number of lattice nodes along the x-axis (periodic)
int Ny; // number of lattice nodes along the y-axis (including two wall nodes)
double tau; // relaxation time
int t_num; // number of time steps (running from 1 to t_num)
...
int wall_num_nodes; // number of IBM nodes along each wall
double wall_width; // channel width
double wall_stiffness; // IBM spring constant
...
int main() {
  ...
  // std::cout << "vel_back: " << vel_top << " , Gamma: " << Gamma << " , zeta: " << zeta << std::endl;
...
void initialize() {
  ...
  for(int X = 0; X < Nx; ++X) {
    ...
    Q_xx[X] = new double[Ny];
    Q_xy[X] = new double[Ny];
  }

  // Initialize the nematic configuration
  ...
  boundaries.push_back(wall_num_nodes);
  ...
  for(int n = 0; n < wall_num_nodes; ++n) {
    ...
    boundaries[boundaries.size() - 1].node[n].y = 1;
    boundaries[boundaries.size() - 1].node[n].y_ref = 1;
...
void read_parameters() {
  ...
  std::string myValues[17];
  std::string myObjects[17];
  ...
  t_num = stoi(myValues[2]); // number of time steps (running from 1 to t_num)
  t_disk = stoi(myValues[3]);
  tau = stof(myValues[4]);

  vel_top = stof(myValues[5]); // velocity of the back wall (in positive x-direction)
  vel_bot = stof(myValues[6]);
  ...
  IBM_stencil = stoi(myValues[13]); // interpolation stencil; available values: 2, 4
  wall_num_nodes = stoi(myValues[14]); // number of IBM nodes along each wall
  wall_width = stof(myValues[15]); // channel width
  wall_stiffness = stof(myValues[16]); // IBM spring constant
...
void LBM() {
  ...
  for(int X = 0; X < Nx; ++X) {
    for(int Y = 1; Y < Ny - 1; ++Y) {
      ...
      const double vel_x = velocity_x[X][Y] + 0.5 * force_x[X][Y] / density[X][Y];
      const double vel_y = velocity_y[X][Y] + 0.5 * force_y[X][Y] / density[X][Y];

      // Compute lattice force (Guo's forcing).
      force_latt[0] = (1 - 0.5 * omega) * weight[0] * (3 * ((   - vel_x) * force_x[X][Y] + (   - vel_y) * force_y[X][Y]));
      force_latt[1] = (1 - 0.5 * omega) * weight[1] * (3 * (( 1 - vel_x) * force_x[X][Y] + (   - vel_y) * force_y[X][Y]) + 9 * (vel_x) * force_x[X][Y]);
      force_latt[2] = (1 - 0.5 * omega) * weight[2] * (3 * ((-1 - vel_x) * force_x[X][Y] + (   - vel_y) * force_y[X][Y]) + 9 * (vel_x) * force_x[X][Y]);
      force_latt[3] = (1 - 0.5 * omega) * weight[3] * (3 * ((   - vel_x) * force_x[X][Y] + ( 1 - vel_y) * force_y[X][Y]) + 9 * (vel_y) * force_y[X][Y]);
      force_latt[4] = (1 - 0.5 * omega) * weight[4] * (3 * ((   - vel_x) * force_x[X][Y] + (-1 - vel_y) * force_y[X][Y]) + 9 * (vel_y) * force_y[X][Y]);
      force_latt[5] = (1 - 0.5 * omega) * weight[5] * (3 * (( 1 - vel_x) * force_x[X][Y] + ( 1 - vel_y) * force_y[X][Y]) + 9 * (vel_x + vel_y) * (force_x[X][Y] + force_y[X][Y]));
      force_latt[6] = (1 - 0.5 * omega) * weight[6] * (3 * ((-1 - vel_x) * force_x[X][Y] + (-1 - vel_y) * force_y[X][Y]) + 9 * (vel_x + vel_y) * (force_x[X][Y] + force_y[X][Y]));
      force_latt[7] = (1 - 0.5 * omega) * weight[7] * (3 * (( 1 - vel_x) * force_x[X][Y] + (-1 - vel_y) * force_y[X][Y]) + 9 * (vel_x - vel_y) * (force_x[X][Y] - force_y[X][Y]));
      force_latt[8] = (1 - 0.5 * omega) * weight[8] * (3 * ((-1 - vel_x) * force_x[X][Y] + ( 1 - vel_y) * force_y[X][Y]) + 9 * (vel_x - vel_y) * (force_x[X][Y] - force_y[X][Y]));
...
void force() {
  ...
  //     const double Q[2] = {Q_xx[X][Y], Q_xy[X][Y]};

  //     // Calculate relaxation first from H = L ∆ Q - a Q - c Q tr(Q^2)
  //     double relaxation[2];
  //     const double trace_square = 2 * SQ(Q[0]) + 2 * SQ(Q[1]);
  //     relaxation[0] = L * laplacian(Q_xx) - (A + U * trace_square) * Q[0];
  //     relaxation[1] = L * laplacian(Q_xy) - (A + U * trace_square) * Q[1];

  //     // anti-symmetric 
  //     // τ = Q H - H Q
  //     double stress_antisymmetric;
  //     stress_antisymmetric = 2 * Q[1] * relaxation[0] - 2 * Q[0] * relaxation[1];

  //     // 0-xx, 1-yy, 2-xy, 3-yz, 4-zx, 5-zz
  //     // σ = ζ Q - ξ (Q + 1/2) H - ξ H (Q + 1/2) + 2 ξ (Q + 1/2) tr(QH) 
  //     //   = ζ Q - 2/3 ξ H + 2 ξ (Q + 1/3) tr(QH) - ξ (Q H + H Q) 
  //     //   = ζ Q - ξ H + 2 ξ Q tr(QH)
  //     double stress_symmetric[2];
  //     const double trace_stress = 2 * relaxation[0] * Q[0] + 2 * relaxation[1] * Q[1];
  //     stress_symmetric[0] = zeta * Q[0] + xi * relaxation[0] + 2 * xi * trace_stress * Q[0];
  //     stress_symmetric[1] = zeta * Q[1] + xi * relaxation[1] + 2 * xi * trace_stress * Q[1];
            
  //     // 0-xx, 1-xy, 2-yy
  //     // σ_ij = - L ∂_j Q_km  ∂_i Q_km
  //     double stress_elastic[3];
  //     stress_elastic[0] = gradient();
  //     stress_elastic[1] = gradient();
  //     stress_elastic[2] = gradient();
  ...
  for(int X = 0; X < Nx; ++X) {
    for(int Y = 1; Y < Ny - 1; ++Y) {
      force_x[X][Y] = zeta * (Q_xx[(X+1) % Nx][Y] - Q_xx[(X-1+Nx) % Nx][Y] + Q_xy[X][Y+1] - Q_xy[X][Y-1]) / 2;
      force_y[X][Y] = zeta * (Q_xy[(X+1) % Nx][Y] - Q_xy[(X-1+Nx) % Nx][Y] - Q_xx[X][Y+1] + Q_xx[X][Y-1]) / 2;
    }
  }
  // relaxation_xx = L * laplacian(Q_xx) - (A + U * trace_square) * Q_xx;
  // relaxation_xy = L * laplacian(Q_xy) - (A + U * trace_square) * Q_xy;

  // force_x = divergence(stress)[0];
  // force_y = divergence(stress)[1];
...
void Q_evolution() {
  ...
  for(int X = 0; X < Nx; ++X) {
    for(int Y = 1; Y < Ny - 1; ++Y) {
      ...
      // Calculate relaxation first from H = L ∆ Q - a Q - c Q tr(Q^2)
      double relaxation[2];
      const double trace = 2 * SQ(Q_xx[X][Y]) + 2 * SQ(Q_xy[X][Y]);
      relaxation[0] = L * (Q_xx[(X+1) % Nx][Y] + Q_xx[(X-1+Nx) % Nx][Y] + Q_xx[X][Y+1] + Q_xx[X][Y-1] - 4 * Q_xx[X][Y]) - (A + U * trace) * Q_xx[X][Y];
      relaxation[1] = L * (Q_xy[(X+1) % Nx][Y] + Q_xy[(X-1+Nx) % Nx][Y] + Q_xy[X][Y+1] + Q_xy[X][Y-1] - 4 * Q_xy[X][Y]) - (A + U * trace) * Q_xy[X][Y];
      
      // Calculate S(nabla u, Q) = ( ξ D + ω ) ( Q + 1/2 ) + ( Q + 1/2 )( ξ D - ω ) - 2 ξ ( Q + 1/2 ) tr(Q nabla u)
      //                         = - 2 ξ Q tr(Q nabla u) + ξ D + ω Q - Q ω 

      // D with central difference
      double strain_rate[2];
      strain_rate[0] = (velocity_x[(X+1) % Nx][Y] - velocity_x[(X-1+Nx) % Nx][Y]) / 2.;
      strain_rate[1] = (velocity_x[X][Y+1] - velocity_x[X][Y-1] + velocity_y[(X+1) % Nx][Y] - velocity_y[(X-1+Nx) % Nx][Y]) / 4.;

      double vorticity;
      vorticity = (velocity_x[X][Y+1] - velocity_x[X][Y-1] - velocity_y[(X+1) % Nx][Y] + velocity_y[(X-1 + Nx) % Nx][Y]) / 4.;


      double advection[2];
      const double traceless_constraint = 2 * Q_xx[X][Y] * strain_rate[0] + 2 * Q_xy[X][Y] * strain_rate[1];
      advection[0] = - 2 * xi * Q_xx[X][Y] * traceless_constraint + xi * strain_rate[0] + vorticity * 2 * Q_xy[X][Y];
      advection[1] = - 2 * xi * Q_xy[X][Y] * traceless_constraint + xi * strain_rate[1] - vorticity * 2 * Q_xx[X][Y];

      // Calculate u dot nabla Q 
      double convection[2];
      convection[0] = velocity_x[X][Y] * 0.5 * (Q_xx[(X+1) % Nx][Y] - Q_xx[(X-1 + Nx) % Nx][Y]) + velocity_y[X][Y] * 0.5 * (Q_xx[X][Y+1] - Q_xx[X][Y-1]);
      convection[1] = velocity_x[X][Y] * 0.5 * (Q_xy[(X+1) % Nx][Y] - Q_xy[(X-1 + Nx) % Nx][Y]) + velocity_y[X][Y] * 0.5 * (Q_xy[X][Y+1] - Q_xy[X][Y-1]);

      // Perform Euler Integration based on Q equation:
      // Q (t + 1) = Q (t) + ∆t (gamma * H + S(nabla u, Q) - u dot nabla Q )
      Q_xx[X][Y] = Q_xx[X][Y] + Gamma * relaxation[0] + advection[0] - convection[0];
      Q_xy[X][Y] = Q_xy[X][Y] + Gamma * relaxation[1] + advection[1] - convection[1];
      
    }
  }

  // Impose no-torque boundary conditions on the directors
  // for(int Y = 0; Y < Ny; ++Y) {
  //   // 
  //   Q_xx[0][Y] = Q_xx[1][Y];
  //   Q_xy[0][Y] = Q_xy[1][Y];

  //   Q_xx[Nx-1][Y] = Q_xx[Nx-2][Y];
  //   Q_xy[Nx-1][Y] = Q_xy[Nx-2][Y];
  //   }
  for(int X = 0; X < Nx; ++X) {
    Q_xx[X][0] = Q_xx[X][1];
    Q_xy[X][0] = Q_xy[X][1];
    
    Q_xx[X][Ny-1] = Q_xx[X][Ny-2];
    Q_xy[X][Ny-1] = Q_xy[X][Ny-2];
    }
...
void spread(vector<IBM_object> & boundary) {
  ...
  for(int i = 0; i < boundary.size(); ++i) {
    ...
    for(int n = 0; n < boundary[i].num_nodes; ++n) {
      ...
      // std::cout << "in spread x: " << boundary[i].node[n].x << ", y: " << boundary[i].node[n].y << std::endl;

      int x_int = 0; 
      int y_int = 0; 

      x_int = (int) (boundary[i].node[n].x - 0.5 + Nx) - Nx;
      y_int = (int) (boundary[i].node[n].y + 0.5);

      // std::cout << "in spread x_int: " << x_int << ", y_int: " << y_int << std::endl;
    ...
    for(int n = 0; n < boundary[i].num_nodes; ++n) {
      ...
      for(int X = X_s; X <= X_e; ++X) {
        for(int Y = Y_s; Y <= Y_e; ++Y) {
          ...
          boundary[i].node[n].vel_x = ((velocity_x[(X + Nx) % Nx][Y] + 0.5 * force_x[(X + Nx) % Nx][Y] / density[(X + Nx) % Nx][Y]) * weight_x * weight_y);
          boundary[i].node[n].vel_y = ((velocity_y[(X + Nx) % Nx][Y] + 0.5 * force_y[(X + Nx) % Nx][Y] / density[(X + Nx) % Nx][Y]) * weight_x * weight_y);
          // std::cout << "in interpolation vel_x: " << boundary[i].node[n].vel_x << ", vel_y: " << boundary[i].node[n].vel_y << std::endl;
...
void update_particle_position(vector<IBM_object> & boundary) {
  ...
  for(int i = 0; i < boundary.size(); ++i) {
    // Update node positions.
    for(int n = 0; n < boundary[i].num_nodes; ++n) {
      // std::cout << "in update vel_x: " << boundary[i].node[n].vel_x << ", vel_y: " << boundary[i].node[n].vel_y << std::endl;
...
void write_fluid_vtk(int time) {
  ...
  for(int Y = 1; Y < Ny - 1; ++Y) {
    for(int X = 0; X < Nx; ++X) {
      output_file << velocity_x[X][Y] + 0.5 * force_x[X][Y] / density[X][Y] << " " << velocity_y[X][Y] + 0.5 * force_y[X][Y] / density[X][Y] << " 0\n";
    }
  }
...
void write_fluid_profile(int time) {
  ...
  for(int X = 0; X < Nx; ++X) {
    for(int Y = 0; Y < Ny; ++Y) {
      output_file << X << " " << Y << " " << density[X][Y] << " " << velocity_x[X][Y] + 0.5 * force_x[X][Y] / density[X][Y] << " " << velocity_y[X][Y] + 0.5 * force_y[X][Y] / density[X][Y] << "\n";
    }
  }
